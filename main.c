...
static const char *help =
        ...
        "\n"
        "OPTIONS:\n"
        "    --activity-type {biking|hiking|running}\n"
...
static int parseArgs(int argc, char **argv, CmdArgs *pArgs)
{
    ...
    for (n = 1, numArgs = argc -1; n <= numArgs; n++) {
        ...
        if (strcmp(arg, "--help") == 0) {
            ...
            val = argv[++n];
            if (strcmp(val, "biking") == 0) {
                pArgs->actType = biking;
            } else if (strcmp(val, "other") == 0) {
                pArgs->actType = other;
            } else if (strcmp(val, "running") == 0) {
                pArgs->actType = running;
            } else {
                invalidArgument(arg, val);
                return -1;
            }
...
static void printTrkPt(TrkPt *p)
{
    ...
    fprintf(stderr, "  latitude=%.10lf longitude=%.10lf elevation=%.10lf time=%.3lf speed=%.3lf grade=%.2lf\n",
            p->latitude, p->longitude, p->elevation, p->timestamp, p->speed, p->grade);
    fprintf(stderr, "}\n");
}
        ...
        if (sscanf(lineBuf, " <type>%d</type>", &type) == 1) {
            ...
            if (pTrkPt->timestamp == 0.0) {
                ...
                if (pArgs->setSpeed == 0.0) {
                    fprintf(stderr, "TrkPt %s:%u is missing its time data !  Is this a GPX route file?\n", pTrkPt->inFile, pTrkPt->lineNum);
                    return -1;
                }
            } else {
                // Timestamps should increase monotonically
                if ((prevTrkPt != NULL) && (pTrkPt->timestamp <= prevTrkPt->timestamp)) {
            ...
            // in the specified range ...
            {
                ...
                if (pArgs->trim) {
                    if (pTrk->index == pArgs->rangeFrom) {
                        // Start trimming
                        trimTrkPts = true;
                        trimmedTime = pTrkPt->timestamp;  // set baseline time
                        pTrk->numTrimTrkPts++;
                        discTrkPt = true;
                    } else if (pTrk->index == pArgs->rangeTo) {
                        // Stop trimming
                        trimTrkPts = false;
                        trimmedTime = pTrkPt->timestamp - trimmedTime + 1;    // total time trimmed out
                        pTrk->numTrimTrkPts++;
                        discTrkPt = true;
                    } else if (trimTrkPts) {
                        // Trim this point
...
static int closeTimeGap(GpsTrk *pTrk, CmdArgs *pArgs)
{
    ...
    while (p2 != NULL) {
        if (p2->index == pArgs->closeGap) {
            timeGap = p2->timestamp - p1->timestamp - 1.0;
        }
...
static int compDataPhase1(GpsTrk *pTrk, CmdArgs *pArgs)
{
    ...
    while (p2 != NULL) {
        ...
        if (pArgs->maxTimeGap != 0) {
            ...
            double timeGap = p2->timestamp - p1->timestamp;
            if (timeGap > pArgs->maxTimeGap) {
                ...
                p2->timestamp = p1->timestamp + pArgs->maxTimeGap;
            }
        }
        ...
        if (pArgs->setSpeed) {
            // Compute the time interval deltaT based on the distance
            // deltaP (in m) and the specified speed (in m/s).
            ...
            p2->timestamp = p1->timestamp + deltaT;
        }
        ...
        if (stopped) {
            ...
            if (!pArgs->verbatim) {
                ...
                p2->adjTime = p2->timestamp - pTrk->stoppedTime;
            }
...
static void printSummary(GpsTrk *pTrk, CmdArgs *pArgs)
{
    ...
    if ((p = pTrk->maxDeltaPTrkPt) != NULL) {
        fprintf(pArgs->outFile, "    maxDeltaP: %.3lf m at TrkPt #%d (%s:%d) : time = %ld s, dist = %.2lf km\n",
                pTrk->maxDeltaP, p->index, p->inFile, p->lineNum, (time_t) (p->timestamp - pTrk->baseTime), (p->distance / 1000.0));
    }
    if ((p = pTrk->maxDeltaTTrkPt) != NULL) {
        fprintf(pArgs->outFile, "    maxDeltaT: %.3lf sec at TrkPt #%d (%s:%d) : time = %ld s, dist = %.2lf km\n",
                pTrk->maxDeltaT, p->index, p->inFile, p->lineNum, (time_t) (p->timestamp - pTrk->baseTime), (p->distance / 1000.0));
    }
    if ((p = pTrk->maxSpeedTrkPt) != NULL) {
        fprintf(pArgs->outFile, "     maxSpeed: %.10lf km/h at TrkPt #%d (%s:%d) : time = %ld s, dist = %.2lf km\n",
                pTrk->maxSpeed, p->index, p->inFile, p->lineNum, (time_t) (p->timestamp - pTrk->baseTime), (p->distance / 1000.0));
    }
    if ((p = pTrk->maxGradeTrkPt) != NULL) {
        fprintf(pArgs->outFile, "     maxGrade: %.2lf%% at TrkPt #%d (%s:%d) : time = %ld s, dist = %.2lf km\n",
                pTrk->maxGrade, p->index, p->inFile, p->lineNum, (time_t) (p->timestamp - pTrk->baseTime), (p->distance / 1000.0));
    }
    if ((p = pTrk->minGradeTrkPt) != NULL) {
        fprintf(pArgs->outFile, "     minGrade: %.2lf%% at TrkPt #%d (%s:%d) : time = %ld s, dist = %.2lf km\n",
                pTrk->minGrade, p->index, p->inFile, p->lineNum, (time_t) (p->timestamp - pTrk->baseTime), (p->distance / 1000.0));
    }
}
