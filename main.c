...
static int compData(GpsTrk *pTrk, CmdArgs *pArgs)
{
    ...
    TAILQ_FOREACH(p2, &pTrk->trkPtList, tqEntry) {
        if (p1 != NULL) {
            ...
            // may use a "smart" recording mode that can have
            // several seconds between points...
            ...
            if (p2->deltaP != 0.0) {
                ...
                if (p2->deltaP > rise) {
                    ...
                    // value is "close to zero" then we just assume we were not
                    // moving.  Otherwise, we just use the data from the previous
                    // point...
                    if (!pArgs->quiet) {
                    ...
                    if (p2->speed < stoppedSpeedThreshold) {
